...
CREATE TABLE rsvps (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    reunion_id UUID REFERENCES reunions(id) ON DELETE CASCADE NOT NULL,
    member_id UUID REFERENCES reunion_members(id) ON DELETE CASCADE,
    name TEXT NOT NULL,
    email TEXT,
...
CREATE TRIGGER update_rsvps_updated_at BEFORE UPDATE ON rsvps FOR EACH ROW EXECUTE PROCEDURE update_updated_at_column();
